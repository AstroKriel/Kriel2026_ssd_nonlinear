...
from jormi.ww_plots import plot_data, plot_manager, add_annotations
...
def get_alpha(fit_params):
    ...
    start_nl_energy = init_energy * np.exp(gamma * start_nl_time)
    sat_energy = 10**log10_sat_energy
    return (sat_energy - start_nl_energy) / (start_sat_time - start_nl_time)


def plot_powerlaw_passing_through(
    ax, domain_bounds, coordinate, slope, num_samples, ls
) -> None:
    x_values = np.linspace(domain_bounds[0], domain_bounds[1], num_samples)
    (x1, y1) = coordinate
    a0 = y1 / x1**slope
    y_values = a0 * x_values**slope
    plot_data.plot_wo_scaling_axis(
        ax=ax, x_values=x_values, y_values=y_values, ls=ls, lw=2
    )


def main() -> None:
    directory = "fits"
    # TODO(AstroKriel): Typo? The line below is invalid
    # io_manager.
