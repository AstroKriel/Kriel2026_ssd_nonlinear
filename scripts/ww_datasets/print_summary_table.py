...
def decimals_from_max_err(e: float) -> int:
  """
...
def round_fixed(x: float, decimals: int) -> str:
  """Format with a fixed number of decimals (no scientific notation)."""
  fmt = f"{{:.{decimals}f}}"
  return fmt.format(x)

def fmt_value_sci(v: float, sig: int = 2) -> str:
...
def from_log10(p50: float, lo: float, hi: float) -> tuple[float, float, float]:
  """
...
def clip_uncertainty(val_str: str, decimals: int) -> str:
  """
...
def fmt_errbar_sci_decimals(v: float, lo: float, hi: float) -> str:
  """
  Put v in convenient scientific basis, pick decimals from largest uncertainty (1 sig digit),
  but cap at 2 decimals. If k==0, no outer parentheses; if k!=0, wrap in parentheses and add ×10^k.
...
def fmt_from_log10_block(d: dict) -> str:
  v, lo, hi = from_log10(d["p50"], d["std_lo"], d["std_hi"])
  return fmt_errbar_sci_decimals(v, lo, hi)

def fmt_from_linear_block(d: dict) -> str:
  return fmt_errbar_sci_decimals(d["p50"], d["std_lo"], d["std_hi"])

def fmt_duration_block(d: dict) -> str:
  """
...
def sims_at_res(count: int, Nres: int) -> str:
...
def build_table(dataset: dict) -> str:
  ...
  for _, suite_stats in dataset.items():
    ...
    mach_str  = str(m_Mach)
    re_str    = str(m_Re)
    nu_str    = fmt_value_sci(nu, sig=2)  # ν: simple scientific, 2 sig figs
    sims_str  = sims_at_res(count, Nres)
