...
def extract_sim_params(
    ...
    if not match_plasma_pattern:
        raise ValueError(f"Could not extract plasma parameters from path: {sim_directory}")
    ...
    if not match_sim_pattern:
        raise ValueError(f"Could not extract resolution from path: {sim_directory}")
...
def plot_and_save_data(
    ...
    axs[0].plot(
        dataset["measured_data"]["time_values"],
        dataset["measured_data"]["rms_Mach_values"],
        color="blue",
    )
    axs[1].plot(
        dataset["measured_data"]["time_values"],
        dataset["measured_data"]["magnetic_energy_values"],
        color="red",
    )
    axs[2].plot(
        dataset["measured_data"]["time_values"],
        numpy.log10(dataset["measured_data"]["magnetic_energy_values"]),
        color="red",
    )
    axs[2].plot(
        dataset["measured_data"]["time_values"],
        numpy.log10(dataset["measured_data"]["kinetic_energy_values"]),
        color="blue",
    )
...
def main():
    ...
    sim_directories = [
        ...
        if io_manager.does_file_exist(
            ...
            any(sim_nres in str(sim_directory)
                for sim_nres in ["288", "576", "1152"]) and "anti" not in str(sim_directory)
        )
    ]
    ## display which directories are going to be looked at
    [print(sim_directory) for sim_directory in sim_directories]
    print(" ")
