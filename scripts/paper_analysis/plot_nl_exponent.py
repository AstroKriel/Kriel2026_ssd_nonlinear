...
def main():
  ...
  for sim_suite in sim_suites:
    ...
    for directory in directories_in_suite:
      ...
      beta_samples = extract_key_param_samples(fitted_posterior_samples)
      all_beta_samples.append(beta_samples)
      p50 = numpy.percentile(beta_samples, 50)
      if (sim_data_dict["plasma_params"]["Mach"] < 1) and (p50 > 1.5):
        print(f"not linear ({p50:.2f}): {directory}/free/stage2_free_fitted_posteriors.png")
      elif (sim_data_dict["plasma_params"]["Mach"] > 1) and (p50 < 1.5):
        print(f"not quadratic ({p50:.2f}): {directory}/free/stage2_free_fitted_posteriors.png")
    if len(all_beta_samples) == 0:
      print("Need to look at:", directory)
      continue
    beta_p50 = numpy.percentile(all_beta_samples, 50)
    beta_color = cmap_beta(norm_beta(beta_p50))
    ...
    ax.errorbar(
      ...
      fmt=marker, color=beta_color, mec="black", markersize=10, capsize=3, zorder=zorder
    )
  ...
  plot_manager.save_figure(fig, f"beta_scaling.png")
