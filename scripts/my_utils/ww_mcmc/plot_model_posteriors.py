## ###############################################################
## DEPENDENCIES
## ###############################################################

import numpy
from jormi.ww_io import io_manager
from jormi.utils import list_utils
from jormi.ww_plots import plot_manager, add_annotations
from jormi.ww_data import compute_stats
from . import base_plotter


## ###############################################################
## PLOTTING ROUTINE
## ###############################################################

class PlotModelPosteriors(base_plotter.BaseMCMCPlotter):

  def plot(self):
    self.num_params  = self.mcmc_routine.num_params
    output_directory = self.mcmc_routine.output_directory
    routine_name     = self.mcmc_routine.routine_name
    verbose          = self.mcmc_routine.verbose
    fig, axs = plot_manager.create_figure(
      num_cols   = self.num_params,
      num_rows   = self.num_params,
      axis_shape = (5,5),
      fig_scale  = 0.75,
    )
    self.self.param_ranges = []
    for row_index in range(self.num_params):
      for col_index in range(self.num_params):
        ax = axs[row_index, col_index]
        if row_index == col_index:
          param_range = self._plot_pdf(ax, row_index)
          self.self.param_ranges.append(param_range)
        elif row_index > col_index:
          self._plot_jpdf(ax, row_index, col_index)
        else: ax.axis("off")
    self._annotate_plot(axs)
    fig_name  = f"{routine_name}_corner_plot.png"
    file_path = io_manager.combine_file_path_parts([ output_directory, fig_name ])
    plot_manager.save_figure(fig, file_path, verbose=verbose)

  def _plot_jpdf(self, ax, row_idx, col_idx):
    posterior_samples = self.mcmc_routine.posterior_samples
    bc_rows, bc_cols, jpdf = compute_stats.estimate_jpdf(
      data_x   = posterior_samples[:, col_idx],
      data_y   = posterior_samples[:, row_idx],
      num_bins = 50
    )
    ax.imshow(
      jpdf,
      origin = "lower",
      aspect = "auto",
      cmap   = "Blues",
      extent = [
        bc_cols[0], bc_cols[-1],
        bc_rows[0], bc_rows[-1],
      ],
    )

  def _plot_pdf(self, ax, param_index):
    posterior_samples = self.mcmc_routine.posterior_samples
    param_labels      = self.mcmc_routine.param_labels
    bin_centers, estimated_pdf = compute_stats.estimate_pdf(
      values   = posterior_samples[:, param_index],
      num_bins = 20
    )
    ax.step(bin_centers, estimated_pdf, where="mid", lw=2, color="black")
    p16, p50, p84 = numpy.percentile(posterior_samples[:, param_index], [16, 50, 84])
    label = f"{param_labels[param_index]} $= {p50:.2f}_{{-{p50-p16:.2f}}}^{{+{p84-p50:.2f}}}$"
    add_annotations.add_text(
      ax          = ax,
      x_pos       = 0.5,
      y_pos       = 1.05,
      label       = label,
      x_alignment = "center",
      y_alignment = "bottom",
    )
    if param_index > 0:
      ax.tick_params(labelleft=False, labelright=True)
    if param_index < posterior_samples.shape[1] - 1:
      ax.set_xticklabels([])
    pdf_threshold = 0.05 * numpy.max(estimated_pdf)
    index_start = list_utils.find_first_crossing(
      values    = estimated_pdf,
      target    = pdf_threshold,
      direction = "rising"
    )
    index_stop = list_utils.find_first_crossing(
      values    = estimated_pdf[index_start:],
      target    = pdf_threshold,
      direction = "falling"
    )
    if index_stop is not None:
      index_stop += index_start
    param_min = bin_centers[index_start]
    param_max = bin_centers[index_stop] if (index_stop is not None) else bin_centers[-1]
    range_width = param_max - param_min
    padding = 2.5/100 * range_width
    padded_range = (
      param_min - padding,
      param_max + padding
    )
    return padded_range

  def _annotate_plot(self, axs):
    num_params   = self.mcmc_routine.num_params
    param_labels = self.mcmc_routine.param_labels
    for row_index in range(num_params):
      for col_index in range(num_params):
        ax = axs[row_index, col_index]
        if col_index > row_index: continue
        if row_index == col_index:
          ax.set_xlim(self.param_ranges[row_index][0], self.param_ranges[row_index][1])
          if col_index > 0: ax.yaxis.tick_right()
        else:
          self._plot_kde(ax, row_index, col_index, self.param_ranges)
          ax.set_xlim(self.param_ranges[col_index][0], self.param_ranges[col_index][1])
          ax.set_ylim(self.param_ranges[row_index][0], self.param_ranges[row_index][1])
          if col_index > 0: ax.set_yticklabels([])
        if row_index == num_params - 1:
          ax.set_xlabel(param_labels[col_index])
        else: ax.set_xticklabels([])
        if col_index == 0:
          ax.set_ylabel(param_labels[row_index])

  def _plot_kde(self, ax, row_index, col_index):
    print(f"Estimating KDE projection for axs[{row_index}][{col_index}]")
    col_grid, row_grid, kde_density = self._compute_2d_kde_projection(
      col_index            = col_index,
      row_index            = row_index,
      grid_resolution      = 20,
      num_marginal_samples = 50,
    )
    ax.contour(
      col_grid,
      row_grid,
      kde_density,
      levels     = 5,
      colors     = "red",
      linewidths = 1.5,
      alpha      = 0.75
    )

  def _compute_2d_kde_projection(self, col_index, row_index, grid_resolution, num_marginal_samples):
    posterior_samples = self.mcmc_routine.posterior_samples
    num_params        = self.mcmc_routine.num_params
    posterior_kde     = self.mcmc_routine.posterior_kde
    marginalized_param_indices = [
      param_index
      for param_index in range(num_params)
      if (param_index != col_index) and (param_index != row_index)
    ]
    col_values = numpy.linspace(
      self.param_ranges[col_index][0],
      self.param_ranges[col_index][1],
      grid_resolution
    )
    row_values = numpy.linspace(
      self.param_ranges[row_index][0],
      self.param_ranges[row_index][1],
      grid_resolution
    )
    col_grid, row_grid = numpy.meshgrid(col_values, row_values)
    col_flat = col_grid.ravel()
    row_flat = row_grid.ravel()
    num_grid_points = col_flat.shape[0]
    marginal_sample_indices = numpy.random.choice(
      posterior_samples.shape[0],
      size    = num_marginal_samples,
      replace = False
    )
    marginalized_samples = posterior_samples[marginal_sample_indices][:, marginalized_param_indices]
    grid_points = numpy.zeros((num_grid_points * num_marginal_samples, num_params))
    grid_points[:, col_index] = numpy.repeat(col_flat, num_marginal_samples)
    grid_points[:, row_index] = numpy.repeat(row_flat, num_marginal_samples)
    grid_points[:, marginalized_param_indices] = numpy.tile(marginalized_samples, (num_grid_points, 1))
    kde_values = posterior_kde(grid_points.T)
    kde_density = kde_values.reshape(num_grid_points, num_marginal_samples).mean(axis=1).reshape(grid_resolution, grid_resolution)
    return col_grid, row_grid, kde_density


## END OF MODULE